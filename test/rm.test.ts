...
describe("rm parse/write", () => {
  ...
  it("appends line items onto a v6 header and parseRm reads them", () => {
    ...
    const mid = appendStrokes(header, [
      {
        points: [
          [0.5, 0.2],
          [0.6, 0.2],
    ...
    expect(parseRm(mid).lines[0]?.points[0]?.x).toBeCloseTo(PAGE_W / 2, 0);
    const svg = linesToSvg(parseRm(mid).lines);
    expect(svg).toContain("702.0");
