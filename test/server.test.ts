...
describe("stdio MCP entry", () => {
  it("handshakes when the tablet SSH port refuses", async () => {
    const transport = new StdioClientTransport({
      command: process.execPath,
      args: ["--import", "tsx", "src/index.ts", "--host", "127.0.0.1", "--port", "1"],
      cwd: root,
      stderr: "pipe",
    });
    const client = new Client({ name: "offline-test", version: "0.0.0" });
    try {
      await client.connect(transport);
      const listed = await client.listTools();
      expect(listed.tools.map((t) => t.name)).toEqual([EXECUTE_TOOL]);
      const result = await client.callTool({
        name: EXECUTE_TOOL,
        arguments: { code: `async () => rme.list({})` },
      });
      ...
      const text = (result.content as { type: string; text?: string }[])
        ...
        .map((c) => c.text ?? "")
        .join("");
      expect(text).toMatch(/ECONNREFUSED|connect/i);
    } finally {
      await client.close();
    }
  });
