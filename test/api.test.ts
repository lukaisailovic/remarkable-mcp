...
describe("library / write / notebook API", () => {
  ...
  it("mkdir/move/rename/delete and memory apply is a no-op", async () => {
    const { fs, rm } = api();
    ...
    expect((await rm.flush()).applied).toBe(false);
    expect((await rm.flush()).applied).toBe(false);
    expect((await rm.refresh()).applied).toBe(false);
    expect(await fs.apply()).toBe(false);
  });
