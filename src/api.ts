...
export function createApi(fs: TabletFs) {
  ...
  // ponytail: one apply per tool call — SSH restarts xochitl, cloud commits once
  const afterWrite = async (r: Rec): Promise<Item> => {
    dirty = true;
    return view(r, paths(await load()));
  };

  const flush = async (): Promise<{ applied: boolean }> => {
    if (!dirty) return { applied: false };
    const applied = await fs.apply();
    dirty = false;
    return { applied };
  };
  ...
  const refresh = async (): Promise<{ applied: boolean }> => {
    const applied = await fs.apply();
    dirty = false;
    return { applied };
  };
